...
export const SALES_PLAYBOOKS: SalesPlaybook[] = [
  ...
  {
    id: 'technician-handoff',
    title: 'Technician Handoff & Trust Building',
    shortTitle: 'Tech Handoff',
    scenario: 'on-site',
    roles: ['technician', 'dispatcher', 'owner'],
    phaseRelevance: ['technician-performance', 'sales-process-call-handling'],
    summary: 'How to execute a professional office-to-field handoff and a strong on-site introduction that builds customer trust before a single tool is touched.',
    whyItWorks: 'The first 90 seconds on-site determine whether the customer trusts the technician\'s recommendations. A confident, professional introduction — by name, by company, with context about the appointment — eliminates the "who is this stranger in my home?" anxiety that makes customers resistant to recommendations.',
    whenToUse: 'Every service call. The handoff starts in dispatch — the tech should arrive knowing the customer\'s name, the problem reported, and any prior service history.',
    steps: [
      {
        stepNumber: 1,
        title: 'Dispatch sets up the tech before arrival',
        notes: 'Dispatcher texts or calls the tech before arrival with: customer name, address, what was reported, any prior service notes, and whether the customer is expecting a specific technician. A tech who arrives knowing the customer\'s name and the reported issue immediately sounds like a professional operation.',
        avoidThis: 'Sending a tech to a call with only an address. The customer already told the office what the problem is — the tech should know it too.',
      },
      {
        stepNumber: 2,
        title: 'Tech-to-customer introduction',
        script: '"Hi, I\'m [Name] from [Company]. I\'m here about the [reported issue]. Thanks for having us out — mind if I start by taking a look at the [system]?"',
        notes: 'Name, company, and reference the specific issue. Use the customer\'s first name if it was shared by dispatch. Asking permission to start is a small gesture that signals respect. Uniform, clean appearance, and a confident handshake form the rest of the first impression.',
        avoidThis: 'Walking past the customer to the equipment without introduction. This is one of the most common technician trust-killers.',
      },
      {
        stepNumber: 3,
        title: 'Explain what you are doing during the diagnostic',
        notes: 'Narrate your diagnostic process briefly as you work: "I\'m going to check [X] first because that\'s usually what causes [symptom]." This keeps the customer engaged and communicates thoroughness. Customers who understand the process are far more receptive to the findings.',
        avoidThis: 'Working silently without any communication. Silence makes customers anxious and distrust the process.',
      },
      {
        stepNumber: 4,
        title: 'Present findings clearly with photos',
        script: '"Here\'s what I found — I took photos so you can see exactly what I\'m talking about. [Show photo.] This is [finding], and what that means for you is [customer-language explanation]."',
        notes: 'Photos turn abstract problems into visible evidence. Every finding should be shown, not just described. Translate every technical finding into what it means for the customer: comfort, safety, cost, or reliability.',
        avoidThis: 'Describing technical findings without showing evidence. Customers who cannot see the problem are more likely to question the diagnosis.',
      },
      {
        stepNumber: 5,
        title: 'Document declined recommendations',
        notes: 'When a customer declines a recommendation, document it in your FSM with a brief note and the customer\'s decision. This protects your company from liability if the declined item causes a failure later, and it creates a follow-up item for your CRM.',
        avoidThis: 'Letting declined recommendations fall through the cracks with no record.',
      },
    ],
    keyPrinciples: [
      'Dispatch prepares the tech — the handoff starts before the tech arrives',
      'Name, company, and reported issue in the first sentence builds instant credibility',
      'Narrate during diagnostics to show thoroughness',
      'Photos turn invisible problems into trusted evidence',
      'Document everything declined — it is both a liability shield and a follow-up opportunity',
    ],
    commonMistakes: [
      'Sending techs to calls without the reported issue details',
      'Walking to the equipment without introducing yourself to the customer first',
      'Working silently — no narration during the diagnostic process',
      'Describing findings without photos to back them up',
      'Not documenting declined recommendations in the FSM',
    ],
    successMetric: 'Customer satisfaction score per tech and upsell attach rate per tech after diagnostic.',
    lockedContent: 'Tech introduction script library · Dispatch-to-tech handoff SOP template · Trade-specific diagnostic narration guide · Declined recommendation documentation protocol · Customer trust-building training module',
  },

  {
    ...
    roles: ['technician', 'csr', 'owner'],
    phaseRelevance: ['followup-reviews-reputation'],
    summary: 'The right moment, method, and message to ask for a Google review in a way that feels natural, earns a high response rate, and generates the detailed reviews that convert new customers.',
    whyItWorks: 'Customers who just had a great service experience are in the highest-possible positive emotional state about your company. The window is 15 minutes to 24 hours. After 24 hours, the emotional peak drops and the review request feels like an obligation rather than a natural expression. Contractors who ask at the right moment, with a direct link, get 5–10x more reviews than those who ask passively.',
    whenToUse: 'After every completed job where the customer expressed satisfaction. The tech should confirm satisfaction at job close — if the customer is happy, the review ask follows immediately.',
    steps: [
      {
        stepNumber: 1,
        title: 'Confirm satisfaction before asking',
        script: '"Before I go — is everything working the way you\'d expect? Any questions about the work I did today?"',
        notes: 'Never ask for a review before confirming the customer is satisfied. An unsatisfied customer who receives a review request will leave a negative review. A satisfied customer who receives a request will leave a positive one.',
        avoidThis: 'Asking for a review before the job is confirmed complete and the customer is satisfied.',
      },
      {
        stepNumber: 2,
        title: 'Ask in person at job completion',
        script: '"If you were happy with the work today, it would really help us out if you\'d leave us a Google review. It only takes about 60 seconds and it makes a big difference for a local company like ours."',
        notes: 'The in-person ask is the highest-converting moment. Keep it conversational — not scripted-sounding. "It makes a big difference for a local company" is genuine, not manipulative, and most customers respond positively to helping a local business.',
        avoidThis: 'Not asking in person, and only relying on automated follow-up texts. The personal ask has a much higher conversion rate.',
      },
      {
        stepNumber: 3,
        title: 'Send a direct link within 30 minutes',
        script: '"Hi [Name], this is [Tech Name] from [Company] — just following up on today\'s service. If you have 60 seconds, we would really appreciate a Google review: [link]. Thank you so much."',
        notes: 'The text goes out within 30 minutes of job completion. Include the direct Google review link (not your homepage — the direct review URL). A direct link removes every barrier between intent and action. Short message wins over long.',
        avoidThis: 'Sending the review link hours later or the next day. The emotional peak passes quickly.',
      },
      {
        stepNumber: 4,
        title: 'Respond to every review within 24 hours',
        notes: 'Respond to all reviews — 5-star and below. 5-star responses: thank the customer by name, reference the specific job, and invite them back. Critical reviews: acknowledge, apologize for the experience, and offer to make it right offline. Google rewards active review engagement with better local search placement.',
        avoidThis: 'Ignoring reviews or only responding to negative ones. Positive review responses are public marketing — future customers read them.',
      },
      {
        stepNumber: 5,
        title: 'Turn strong reviews into social proof content',
        notes: 'Screenshot notable reviews and post them to Facebook and Google Business Profile with a brief thank-you note. Ask for permission if you use a customer\'s full name. This creates content, reinforces positive public perception, and shows future customers that you value feedback.',
        avoidThis: 'Collecting reviews and never doing anything with them beyond the aggregate star rating.',
      },
    ],
    keyPrinciples: [
      'Confirm satisfaction before asking — every time',
      'The in-person ask is the highest-converting moment',
      'Send the direct Google link within 30 minutes of job close',
      'Respond to every review — positive and negative',
      'Turn strong reviews into social proof content',
    ],
    commonMistakes: [
      'Asking for reviews before confirming customer satisfaction',
      'Sending a review link to your homepage instead of the direct Google review URL',
      'Waiting until the next day to send the follow-up text',
      'Not responding to positive reviews — they are public marketing opportunities',
      'Relying only on automated requests without the in-person ask',
    ],
    successMetric: 'Review count per month and average star rating over trailing 90 days.',
    lockedContent: 'Review request text templates by trade and call type · Direct Google review link setup guide · Review response template library · Social proof content system using reviews · Review milestone campaign guide (25, 50, 100 reviews)',
  },

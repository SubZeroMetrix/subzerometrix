...
import {
  ...
  ChevronUp, Lightbulb, DollarSign, TriangleAlert, Thermometer, Zap,
} from 'lucide-react'
...
import { getTempStatus, TEMP_CONFIG, SCORE_IMPACT_PER_TAB, type GrowthRoadmapTab, type GrowthVendor, type TempStatus } from '@/lib/growthRoadmap'
...
// ── Temperature badge ─────────────────────────────────────────────────────────
function TempBadge({ status }: { status: TempStatus }) {
  const tc = TEMP_CONFIG[status]
  return (
    <span className="flex-shrink-0 inline-flex items-center gap-1 text-[10px] font-medium px-2 py-1 rounded-sm"
...
// ── Growth vendor card ────────────────────────────────────────────────────────
function GrowthVendorCard({ vendor }: { vendor: GrowthVendor }) {
  const partner = vendor.affiliateId
    ? AFFILIATE_PARTNERS.find(p => p.id === vendor.affiliateId)
    : null
  const href = partner ? affiliateUrl(partner) : vendor.url
  return (
    <div className="rounded-sm p-3"
      ...
      <p className="text-[10px] text-brand-silver/50 font-mono mb-1">{vendor.category}</p>
      <p className="text-[11px] text-brand-silver leading-relaxed mb-1">{vendor.whatItHelps}</p>
      <p className="text-[11px] mb-2 leading-relaxed" style={{ color: '#1D9E75' }}>
...
// ── Growth tab panel ──────────────────────────────────────────────────────────
function GrowthTabPanel({
  tab, result, completed, onToggleComplete, bandColor,
}: {
  ...
  const [showVendors, setShowVendors] = useState(false)
  const tempSt = getTempStatus(tab, result, completed)
  const tc = TEMP_CONFIG[tempSt]

  return (
    <div className="rounded-sm overflow-hidden"
      style={{
        border: completed ? '1px solid rgba(29,158,117,0.4)' : `1px solid ${tc.color}40`,
        background: completed ? 'rgba(29,158,117,0.05)' : 'rgba(10,22,40,0.5)',
      }}>

      {/* Thermometer bar */}
      <div className="h-1" style={{ background: 'rgba(168,184,204,0.08)' }}>
        <div className="h-full transition-all duration-700"
          style={{ width: `${tc.barPct}%`, background: tc.color }} />
      </div>

      {/* Header */}
      <div className="px-4 pt-3 pb-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
        <div className="flex items-start justify-between gap-3 mb-1">
          <h3 className="font-display text-xl tracking-wider text-brand-white leading-none">
            {tab.title.toUpperCase()}
          </h3>
          <TempBadge status={tempSt} />
        </div>
        <p className="text-[11px] text-brand-silver/60">{tab.subtitle}</p>
      </div>
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        <div>
          ...
          <p className="text-[12px] text-brand-silver leading-relaxed">{tab.whyItMatters}</p>
        </div>

        {/* Current gap */}
        <div className="px-3 py-2.5 rounded-sm"
          style={{ background: `${tc.color}0D`, border: `1px solid ${tc.color}30` }}>
          <p className="font-mono text-[9px] tracking-widest uppercase mb-1" style={{ color: tc.color }}>
            Current gap
          </p>
          <p className="text-[11px] text-brand-silver/80 leading-relaxed">{tab.currentWeaknessSignal}</p>
        </div>

        {/* Primary + Alternative actions */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div className="rounded-sm p-3"
            style={{ background: 'rgba(29,158,117,0.07)', border: '1px solid rgba(29,158,117,0.25)' }}>
            <p className="font-mono text-[9px] tracking-widest uppercase mb-0.5" style={{ color: '#1D9E75' }}>
              Primary action
            </p>
            <p className="font-mono text-[8px] tracking-wider uppercase text-brand-silver/40 mb-1.5">
              {tab.primaryAction.timeframe}
            </p>
            <p className="text-[12px] font-semibold text-brand-white mb-1 leading-snug">
              {tab.primaryAction.title}
            </p>
            <p className="text-[11px] text-brand-silver leading-relaxed">{tab.primaryAction.description}</p>
          </div>

          <div className="rounded-sm p-3"
            style={{ background: 'rgba(74,144,217,0.07)', border: '1px solid rgba(74,144,217,0.2)' }}>
            <p className="font-mono text-[9px] tracking-widest uppercase mb-0.5 text-brand-accent">
              Alternative action
            </p>
            <p className="font-mono text-[8px] tracking-wider uppercase text-brand-silver/40 mb-1.5">
              {tab.alternativeAction.timeframe}
            </p>
            <p className="text-[12px] font-semibold text-brand-white mb-1 leading-snug">
              {tab.alternativeAction.title}
            </p>
            <p className="text-[11px] text-brand-silver leading-relaxed">{tab.alternativeAction.description}</p>
          </div>
        </div>
        ...
        <div className="flex items-start gap-2 px-3 py-2.5 rounded-sm"
          ...
          <Zap className="w-3.5 h-3.5 text-yellow-500/70 flex-shrink-0 mt-0.5" />
          <div>
            <p className="font-mono text-[9px] tracking-widest uppercase text-yellow-500/70 mb-0.5">Quick win</p>
            <p className="text-[11px] text-brand-silver leading-relaxed">{tab.quickWin}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            <p className="font-mono text-[9px] tracking-widest uppercase text-brand-silver/50 mb-1">
              What to measure
            </p>
            <p className="text-[11px] text-brand-silver leading-relaxed">{tab.whatToMeasure}</p>
          </div>
          <div>
            ...
            <p className="text-[11px] text-brand-silver leading-relaxed">{tab.salesImpact}</p>
          </div>
        </div>

        {/* Vendors */}
        {tab.vendors.length > 0 && (
          <div>
            <button onClick={() => setShowVendors(v => !v)}
              ...
              {showVendors
                ? 'Hide tools'
                : `See ${tab.vendors.length} tool${tab.vendors.length > 1 ? 's' : ''} for this area`}
            </button>
            {showVendors && (
              <div className="space-y-2">
                <p className="text-[9px] text-brand-silver/50 leading-relaxed px-1 pb-1"
                  style={{ borderBottom: '1px solid rgba(168,184,204,0.1)' }}>
                  Some links may earn a commission. Recommendations are based on your needs, not commission rates.{' '}
                  <Link href="/affiliate-disclosure" className="text-brand-accent hover:underline">
                    Full disclosure →
                  </Link>
                </p>
                {tab.vendors.map(v => <GrowthVendorCard key={v.name} vendor={v} />)}
              </div>
            )}
          </div>
        )}
        ...
        <div className="flex items-center gap-3 pt-1">
          <div className="flex items-center gap-1.5 text-[10px] text-brand-silver/50">
            ...
            <span>+{tab.scoreImpact} projected pts when complete</span>
          </div>
          <button onClick={onToggleComplete}
            ...
            {completed ? '✓ Complete — undo' : 'Mark complete'}
          </button>
        </div>
      </div>
    </div>
